...
const Ico = {
  ...
  youtube: <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="1" y="4" width="22" height="16" rx="5" fill="#ff3b30"/><path d="M10 8.5v7l6-3.5z" fill="#fff"/></svg>,
  calendar: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="5" width="18" height="16" rx="1"/><path d="M3 10h18M8 3v4M16 3v4"/></svg>,
  pin: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 21s7-6.5 7-11.5A7 7 0 0 0 5 9.5C5 14.5 12 21 12 21z"/><circle cx="12" cy="9.5" r="2.3"/></svg>,
...
function GNav({ active = 'ニュース' }) {
  const items = ['TOP', 'ニュース', 'ツアー', 'テニス用品', 'レッスン', 'スクール', 'サークル', '大会', 'イベント', 'レンタルコート', 'コーチ求人'];
  return (
...
// ─── トーナメント表 (ツアー速報 / ニュース で共通利用) ───────
function TournamentTable({ title = 'トーナメント表 · 準決勝', matches, final }) {
  return (
    <div>
      ...
      <div style={{border:'1px solid var(--wf-line)', background:'#fff'}}>
        {matches.map((m, i) => (
          <div key={i} style={{
            padding:'8px 10px',
            borderBottom: i < matches.length - 1 ? '1px solid var(--wf-line)' : 0,
            fontSize:11.5,
          }}>
...
// ─── スマホフレーム (シンプル・軽量版) ─────────────────────
function Phone({ children, width = 375, height }) {
  return (
    <div style={{
      width, ...(height ? { height } : {}),
      background: '#fff',
      ...
      {/* 高さ指定なし = ページ全体を折り返さず表示（内部スクロールなし） */}
      <div className="wf-frame" style={{
        ...(height ? { height: height - 28, overflow: 'auto' } : { overflow: 'visible' }),
      }}>
        {children}
      </div>
    </div>
